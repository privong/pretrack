...
# will update to use this one at some point. better accuracy control
echo "Getting initial raw position fix..."
pos=$(luna-send -n 2 palm://com.palm.location/getCurrentPosition '{"appId": "magellan", "accuracy:3", "responseTime:1", "maximumAge:0"}'2>&1)
echo $pos
echo "Getting better position fix"
pos=$(luna-send -n 2 palm://com.palm.location/getCurrentPosition '{"appId": "magellan", "accuracy:1", "responseTime:3", "maximumAge:0"}' 2>&1 | tail -1 | cut -d, -f4,5,6,7,8,9,10 | sed -r 's/[^-\.0-9,]//g')
echo $pos
